...
class KrookModule(modules.base.BaseModule):
    ...
    def __init__(self, module_id: int):
        ...
        self.alert_states = ['disassociated', 'deauthenticated']
        if context.kr00k_alert_unknown:
            alert_states.append(None)
            
    def _try_decrypt(self, frame, sm: StatManager, ctx: dict, alert: dict) -> bool:
    ...
    def _detect_kr00k(self, frame: Packet, sm: StatManager, ctx: dict) -> dict|None:
        # not Dot11CCMP, not kr00k, according to the r00kie kr00kie script
        if not frame.haslayer(Dot11CCMP):
            return None

        state = sm.sta_state.get(frame.addr2)
    ...
    def  _on_frame(self, frame: Packet, sm: StatManager, ctx: dict) -> list:
        alerts = []
        alert = self._detect_kr00k(frame, sm, ctx)
